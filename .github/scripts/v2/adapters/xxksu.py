...
PATCH11_FILE_INDEXES: Mapping[str, str] = {
    ...
    "kernel/feature/kernel_umount.c": "index a931624..fe988bf 100644",
...
PATCH11_PREAMBLE: Tuple[str, ...] = (
    ...
    " kernel/feature/kernel_umount.c        |  56 ++++++---",
    ...
    " 10 files changed, 545 insertions(+), 37 deletions(-)",
    "",
)
...
def get_patch11_operation_specs() -> Tuple[XxksuOperationSpec, ...]:
    ...
    return (
        ...
        XxksuOperationSpec(
            ...
            file_path='kernel/feature/kernel_umount.c',
            spec=AnchorSpec('kernel/feature/kernel_umount.c', '\t// check old process\'s selinux context, if it is not zygote, ignore it!\n\t// because some su apps may setuid to untrusted_app but they are in global mount namespace\n\t// when we umount for such process, that is a disaster!\n\t// also handle case 4 and 5\n\tbool is_zygote_child = is_zygote(old);\n\tif (!is_zygote_child) {\n\t\tpr_info("handle umount ignore non zygote child: %d\\n", current->pid);\n\t\treturn 0;\n\t}\n\n\tset_thread_flag(TIF_KSU_UNMOUNTABLE);\n\n', context_before=('if (!ksu_uid_should_umount(new_uid) && !is_isolated_process(new_uid))', 'return 0;'), context_after=()),
            placement=Placement.REPLACE,
            payload='',
            section_context='static inline int ksu_handle_umount(struct cred *new, const struct cred *old)',
            context_before_count=3,
            context_after_count=3,
            context_before_offset=3,
            diff_body=(('-', "\t// check old process's selinux context, if it is not zygote, ignore it!"), ('-', '\t// because some su apps may setuid to untrusted_app but they are in global mount namespace'), ('-', '\t// when we umount for such process, that is a disaster!'), ('-', '\t// also handle case 4 and 5'), ('-', '\tbool is_zygote_child = is_zygote(old);'), ('-', '\tif (!is_zygote_child) {'), ('-', '\t\tpr_info("handle umount ignore non zygote child: %d\\n", current->pid);'), ('-', '\t\treturn 0;'), ('-', '\t}'), ('-', ''), ('-', '\tset_thread_flag(TIF_KSU_UNMOUNTABLE);'), ('-', '')),
        ),
        XxksuOperationSpec(
